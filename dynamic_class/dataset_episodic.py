from typing import List, Callable, Mapping, Sequence, Tuple, Iterator
...
class EpisodesDataset(IterableDataset):
    ...
    def __init__(
        ...
        for item in data:
            source, target = item["inputs"], item["targets"]
    ...
    def __iter__(self) -> Iterator[Tuple[torch.Tensor, ...]]:
        ...
        for _ in range(self.n_episodes):
            ...
            if self.n_classes is not None:
                classes = list(np.random.permutation(self.all_classes))[
                    : self.n_classes
                ]
            ...
            for i, target_class in enumerate(classes):
                ...
                supports.extend(
                    [(examples[j][0], i) for j in indices[: self.n_support]]
                )

                if self.balance_query:
                    query_indices = indices[
                        self.n_support : self.n_support + self.n_query
                    ]
            ...
            query_source = [torch.Tensor(x) for x in query_source]
            support_source = [torch.Tensor(x) for x in support_source]

            query_source = pad_sequence(
                query_source, batch_first=True, padding_value=self.pad
            )
            query_target = torch.tensor(query_target)

            support_source = pad_sequence(
                support_source, batch_first=True, padding_value=self.pad
            )
            ...
            yield {
                "query": query_source.long(),
                "query_label": query_target.long(),
                "support": support_source.long(),
                "support_label": support_target.long(),
            }
