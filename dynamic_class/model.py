...
class PrototypicalTextClassifier(nn.Module):
    ...
    def __init__(
       ...
       Extra keyword arguments are passed to the RNNEncoder.
        ...
        self.encoder = RNNEncoder(input_size=self.embedding.embedding_dim, **kwargs)

    def encode(self, text: Tensor) -> Tensor:
        padding_mask = (text != self.padding_idx).byte()
        text_embeddings = self.embedding_dropout(self.embedding(text))
        text_encoding = self.encoder(text_embeddings, padding_mask=padding_mask)
    ...
    def forward(
        ...
        """
        if len(query.size()) != 2:
            raise ValueError("Please check query size")
        if support is not None and len(support.size()) != 2:
            raise ValueError("Please check support size")
